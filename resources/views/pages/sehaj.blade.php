<!DOCTYPE html>
<html>

<head>
    <title>About Us</title>
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css"
        integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">
</head>

<body>
<nav class="navbar navbar-expand-lg navbar-dark ">
       
       <div class="collapse navbar-collapse" id="navbarNavDropdown">
       <a class="dropdown-toggle " type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
           Team
       </a>
       <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
           <a class="dropdown-item" href="/nathan">Nathan Cherry</a>
           <a class="dropdown-item" href="/ian">Ian Straatman</a>
           <a class="dropdown-item" href="/marko">Marko Milovic</a>
           <a class="dropdown-item" href="/sehaj">Sehaj Khaira</a>
           <a class="dropdown-item" href="/ehu">Ehabuddin Mohammed</a>
           <a class="btn btn-success ml-4 mr-4" style="width: 209px;" href="{{ url('/join') }}">Apply Now!</a>
       </div>
                   <a class="nav-link" href="/">Home <span class="sr-only">(current)</span></a>
               

               
       <a  class="nav-link link-dark" href="/mission">Mission<span class="sr-only">(current)</span></a>
       <a  class="nav-link" href="/history">History<span class="sr-only">(current)</span></a>
      
       <a  class="nav-link" href="/legal">Legal<span class="sr-only">(current)</span></a>
       <a  class="nav-link" href="/contact">Contact<span class="sr-only">(current)</span></a>
       
       </div>
   </nav>

    <div class="mt-5 ml-5 mr-5 mb-5">

        <h1 class="text-center">About</h1> <br>

        <div class="container">
            <div class="row">

                <div class="text-center  border border-secondary p-c5">
                    <h1 class="text-center">Meet Sehaj</h1>
                    <p class="text-center">

                    I, Sehaj Khaira, am a Teaching Assistant at the University of Windsor and am currently 
                    starting my fourth year of a Computer Science undergraduate degree. I have multiple years 
                    of industry experience holding IT positions at different companies,
                     mainly with Ground Effects and University of Windsor. 
                    </p>
                    <p class="text-center">

                    This course is interestomg to me as I have been wanting to work in industry as a Web Developer for the next few years.
                     Although I do have experience with a large portion of the material that will likely be covered during the course
                     , you can always learn something new. Hopefully this course will further my web development skills. 
                     One of my goals  is to familiarize myself with the Node.js library. 
</p>

                </div>
            </div>
        </div>

    </div>

    <script src="https://code.jquery.com/jquery-3.2.1.slim.min.js"
        integrity="sha384-KJ3o2DKtIkvYIK3UENzmM7KCkRr/rE9/Qpg6aAZGJwFDMVNA/GpGFF93hXpG5KkN"
        crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.12.9/umd/popper.min.js"
        integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q"
        crossorigin="anonymous"></script>
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/js/bootstrap.min.js"
        integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl"
        crossorigin="anonymous"></script>

</body>

</html>